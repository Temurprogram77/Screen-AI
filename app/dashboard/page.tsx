...
function DashboardContent() {
  ...
  const activeJobId = searchParams.get("jobId") || currentJobId;
  ...
  // Redirect to home if no candidates exist locally or in DB
  useEffect(() => {
    if (!dbLoading && candidates.length === 0 && !activeJobId) {
      router.replace("/");
    }
  }, [candidates.length, dbLoading, activeJobId, router]);

  const handleReset = () => {
    if (confirm("Start a new screening session? (Your saved jobs in the database remain safe)")) {
      resetAll();
      router.push("/");
    }
  };
  ...
  if (candidates.length === 0 && !activeJobId) {
